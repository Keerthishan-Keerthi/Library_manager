...
public class view_loan extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addTextToRowWithStyle(headerRow, "Access No.");
        addTextToRowWithStyle(headerRow, "Branch Id");
        addTextToRowWithStyle(headerRow, "Card No.");
        addTextToRowWithStyle(headerRow, "Date out");
        addTextToRowWithStyle(headerRow, "Date due");
        addTextToRowWithStyle(headerRow, "Date returned");
        tableLayout.addView(headerRow);

        // Add data rows
        Cursor cursor = db.rawQuery(" SELECT * FROM loan_table ", null);
        if (cursor.moveToFirst()) {
            do {
                ...
                addTextToRowWithStyle(row, cursor.getString(5)); // Column 6
